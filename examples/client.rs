...
const RPC_URL: &str = "http://127.0.0.1:8899"; // For Airdrop
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Created transaction with signature: {}", transaction.signatures[0]);

    send_transaction(&session, transaction).await?;
...
async fn connect_to_bifrost() -> anyhow::Result<web_transport_quinn::Session> {
    ...
    let url = Url::parse(BIFROST_URL)
        .context(format!("Invalid Bifrost URL: {}", BIFROST_URL))?;
...
async fn create_test_transaction() -> anyhow::Result<Transaction> {
    ...
    request_and_confirm_airdrop(&rpc_client, &payer).await?;
    
    let balance = rpc_client.get_balance(&payer.pubkey())?;
    println!("Payer balance: {} SOL", balance as f64 / LAMPORTS_PER_SOL as f64);
...
async fn request_and_confirm_airdrop(
    ...
    println!("Requesting airdrop of {} SOL", airdrop_amount / LAMPORTS_PER_SOL);
...
fn build_transfer_transaction(
    ...
    let transfer_amount = rent_exempt + 1_000_000; // Rent-exempt + 1 million lamports
